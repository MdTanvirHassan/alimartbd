...
class SteadfastCourierController extends Controller
{
    ...
    {
        $this->base_url = env("steadfast_base_url");
    }
    ...
    public function create_order($order_id)
    {
        ...
        $data = array(
            ...
            'recipient_address' => $shipping_address->address ?? 'N/A',
        ...
        if ($responseData['status'] == 200 && isset($responseData['consignment'])) {
            ...
            $consignments->invoice = $consignmentData['invoice'];
            $consignments->tracking_code = $consignmentData['tracking_code'];
            $consignments->recipient_name = $consignmentData['recipient_name'];
            $consignments->recipient_phone = $consignmentData['recipient_phone'];
            $consignments->recipient_address = $consignmentData['recipient_address'];
            $consignments->cod_amount = $consignmentData['cod_amount'];
            $consignments->status = $consignmentData['status'];
            $consignments->note = $consignmentData['note'];
            $consignments->save();
        }
    }
