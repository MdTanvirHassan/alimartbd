...
class PathaoCourierController extends Controller
{
    ...
    public function saveOrder(Request $request){
        ...
        $item_weight = 0;
        if($request->item_weight==0 || $request->item_weight <= 0){
            $item_weight = 0.1;
        }else{
            $item_weight = $request->item_weight;
        }

        $recipient_zone = isset($shipping_address->zone) ? $shipping_address->zone : null;
        $recipient_area = isset($shipping_address->area) ? $shipping_address->area : null;
    ...
    public function saveStore(Request $request){
        ...
        if(isset($responseData['data']['store_id'])){
            $store_id = $responseData['data']['store_id'];
            $store_name = $responseData['data']['store_name'];
        
            $pathao_stores = new Pathao_store();
            $pathao_stores->store_id = $store_id;
            $pathao_stores->name = $store_name;
            $pathao_stores->contact_name = $request->contactName;
            $pathao_stores->contact_number = $request->contactNumber;
            $pathao_stores->secondary_contact = $request->secondaryContact;
            $pathao_stores->address = $request->address;
            $pathao_stores->city_id = $request->city_id;
            $pathao_stores->zone_id = $request->zone_id;
            $pathao_stores->area_id = $request->area_id;
            $pathao_stores->save();
            ...
            flash(translate('Store creation Failed'))->error();
    ...
    public function bulk_order(Request $request){
        ...
        $orders = [];
        $Pathao_store = Pathao_store::where('name','Star Abode')->value('store_id');
        $storeId = $Pathao_store;
        $itemType = 2;
        $deliveryType = 48;
